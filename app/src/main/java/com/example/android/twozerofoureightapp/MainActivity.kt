...
import com.example.android.engine.*
...
//    }
    ...
    private fun paintCell(obj: Any, value: Int) {
        ...
        var bgCol = resources.getColor(R.color.white, null)

        when (value) {
            2 -> {
                txCol = resources.getColor(R.color.t_light_text, null)
                bgCol = resources.getColor(R.color.t2_bg, null)
            }
            4 -> {
                txCol = resources.getColor(R.color.t_light_text, null)
                bgCol = resources.getColor(R.color.t4_bg, null)
            }
            8 -> {
                txCol = resources.getColor(R.color.t_light_text, null)
    ...
    override fun userPB(score: Int) {
        Toast.makeText(this, resources.getString(R.string.personal_best_toast_message),
            Toast.LENGTH_SHORT).show()
        this.dataStore.putHighscore(score)
        this.highScore = score
        val tv_score_best = findViewById<TextView>(R.id.tv_score_best)
        tv_score_best.text = highScore.toString()
    }

    override fun userScoreChanged(score: Int) {
        val tv: TextView = findViewById(R.id.tv_score)
        tv.text = StringBuffer().append(score)
    }
